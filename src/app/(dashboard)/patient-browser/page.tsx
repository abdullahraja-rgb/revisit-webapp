          ...
          <div className="relative" ref={dropdownRef}>
              ...
              {isDropdownOpen && (
                  <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg border z-10">
                      <a href="#" onClick={(e) => { e.preventDefault(); handleCreatePatient(); setDropdownOpen(false); }} className="block px-4 py-2 text-sm text-neutral-700 hover:bg-neutral-100">New Patient</a>
                      <a href="#" onClick={(e) => { e.preventDefault(); handleCreateCarer(); setDropdownOpen(false); }} className="block px-4 py-2 text-sm text-neutral-700 hover:bg-neutral-100">New Carer</a>
                  </div>
              )}
          </div>
          ...
          <div className="p-4 border-b border-neutral-200">
              <form onSubmit={handleSearch} className="relative">
                  ...
                  <input 
                      ...
                      className="pl-9 pr-4 py-2 w-full md:w-80 border border-neutral-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary" 
                  />
              </form>
          </div>
              ...
              <table className="min-w-full divide-y divide-neutral-200">
                  ...
                  <tbody className="bg-white divide-y divide-neutral-200">
                      {isLoading || isSearching ? (
                          <tr><td colSpan={6} className="text-center p-8 text-neutral-500"><Loader className="w-6 h-6 animate-spin mx-auto text-primary" /></td></tr>
                          ...
                          patientsToDisplay.map((patient) => {
                              ...
                              return (
                                  <tr key={patient.id} onClick={() => handleViewProfile(patient)} className="hover:bg-neutral-50 cursor-pointer">
                                      ...
                                      <td className="table-body-cell capitalize text-neutral-600">{patient.gender || 'N/A'}</td>
                                      <td className="table-body-cell text-neutral-600">{patient.birthDate || 'N/A'}</td>
                                      <td className="table-body-cell">
                                          <span className={`px-2 py-1 text-xs font-semibold rounded-full ${patient.active ? 'bg-success-light text-success' : 'bg-warning-light text-warning-dark'}`}>{patient.active ? 'active' : 'pending'}</span>
                                      </td>
                          ...
                          <tr><td colSpan={6} className="text-center p-12 bg-neutral-50"><Inbox className="w-12 h-12 mx-auto text-neutral-400" /><h3 className="mt-2 text-lg font-medium text-neutral-900">No Patients Found</h3><p className="mt-1 text-sm text-neutral-500">{hasSearched ? "Try adjusting your search criteria." : "Create a new patient to get started."}</p></td></tr>
                      )}
                  </tbody>
              </table>
          </div>
          <div className="p-4 flex items-center justify-between text-sm text-neutral-600">
              <p>Showing 1 to {patientsToDisplay.length} of {patientsToDisplay.length} entries</p>
              <div className="flex items-center space-x-1">
                  <button className="px-2 py-1 border rounded-md hover:bg-neutral-100" disabled>&laquo;</button>
                  <button className="px-3 py-1 border rounded-md bg-primary-light text-primary font-bold">1</button>
                  <button className="px-2 py-1 border rounded-md hover:bg-neutral-100" disabled>&raquo;</button>
              </div>
          </div>
      </div>
    </div>
