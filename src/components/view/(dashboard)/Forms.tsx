  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 max-h-[80vh] overflow-y-auto pr-4">
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div><label htmlFor="nhsNumber" className="form-label">NHS Number</label><input type="text" value={nhsNumber} onChange={(e) => setNhsNumber(e.target.value)} id="nhsNumber" placeholder="e.g., 123 456 7890" className="form-input" /></div>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          ...
          <div><label htmlFor="country" className="form-label">Country</label><input type="text" value={country} onChange={(e) => setCountry(e.target.value)} id="country" className="form-input" /></div>
        </div>
      </div>
...
export const CreateCarerForm: React.FC<FormProps> = ({ onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Get the display text from the selected relationship option
    ...
    const result = await createCarer({
      firstName, lastName, gender, phone, patientId, patientName, relationship, relationshipDisplay
    });
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} id="carerPhone" className="form-input" />
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div>
            ...
            <input type="text" value={patientId} onChange={(e) => setPatientId(e.target.value)} id="patientId" placeholder="e.g., patient-arthur-miller-abc" className="form-input" required />
          </div>
           <div>
            <label htmlFor="patientName" className="form-label">Patient Display Name</label>
            <input type="text" value={patientName} onChange={(e) => setPatientName(e.target.value)} id="patientName" placeholder="e.g., Arthur Miller" className="form-input" />
          </div>
          <div className="sm:col-span-2">
            <label htmlFor="relationship" className="form-label">Relationship to Patient</label>
            <select value={relationship} onChange={(e) => setRelationship(e.target.value)} id="relationship" name="relationship" className="form-input">
...
export const CreateAssessmentForm: React.FC<FormProps> = ({ onClose, patient: preselectedPatient }) => {
  ...
  useEffect(() => {
    if (assessmentType === 'Standard Home Safety Check') {
      const generatedBlocks = homeSafetyTaskTemplate.map(block => ({
        ...block,
        id: crypto.randomUUID(),
        subTasks: block.subTasks.map(subTask => ({ ...subTask, id: crypto.randomUUID() })),
      }));
      setTaskBlocks(generatedBlocks);
      setOpenBlockIds(generatedBlocks.map(b => b.id)); // Auto-open generated blocks
    } else {
      setTaskBlocks([]);
    }
  }, [assessmentType]);
  ...
  const addTaskBlock = () => {
    ...
    setTaskBlocks(prev => [...prev, { id: newBlockId, title: '', subTasks: [] }]);
    setOpenBlockIds(prev => [...prev, newBlockId]); // Auto-open the new block
  };

  const addSubTask = (blockId: string) => {
    const newSubTask: SubTask = { id: crypto.randomUUID(), description: '', type: 'custom', notes: '' };
    setTaskBlocks(prev => prev.map(block => block.id === blockId ? { ...block, subTasks: [...block.subTasks, newSubTask] } : block));
  };
  
  const handleBlockTitleChange = (blockId: string, title: string) => {
    setTaskBlocks(prev => prev.map(block => block.id === blockId ? { ...block, title } : block));
  };

  const handleSubTaskDescriptionChange = (blockId: string, subTaskId: string, description: string) => {
     setTaskBlocks(prev => prev.map(block => block.id === blockId ? { ...block, subTasks: block.subTasks.map(sub => sub.id === subTaskId ? { ...sub, description } : sub) } : block));
  };
  
  const handleNoteChange = (blockId: string, subTaskId: string, notes: string) => {
    setTaskBlocks(prev => prev.map(block => block.id === blockId ? { ...block, subTasks: block.subTasks.map(sub => sub.id === subTaskId ? { ...sub, notes } : sub) } : block));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Use the preselected patient's ID if available, otherwise use the input field's value
    const patientId = preselectedPatient ? preselectedPatient.id : patient;
    
    const assessmentData = {
      serviceRequest: { patient: patientId, assessmentType, performerType, performerName, startDate, endDate },
    ...
    <form onSubmit={handleSubmit} className="space-y-8 max-h-[80vh] overflow-y-auto pr-4">
      ...
      <div className="space-y-6">
        <div>
            ...
            <input type="text" placeholder="Search patient by name or DOB" className="form-input" value={patient} onChange={(e) => setPatient(e.target.value)} required disabled={!!preselectedPatient} />
        </div>
        <div>
