...
const PatientProfileModal: React.FC<PatientProfileModalProps> = ({ patient, onCreateAssessment }) => {
  const [activeTab, setActiveTab] = useState<'Overview' | 'Observations' | 'Assessments'>('Overview'); // Default to Overview
      ...
      <div className="flex items-center mb-6">
        ...
        <div className="ml-4">
          <h2 className="text-2xl font-bold text-gray-900 flex items-center">
            ...
            <span className={`ml-3 text-xs font-medium px-2.5 py-1 rounded-full ${patient.active ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>{patient.active ? 'active' : 'pending'}</span>
          </h2>
      ...
      <div>
        ...
        {activeTab === 'Observations' && (
          <div>
            ...
            {isLoading ? (
                <div className="flex justify-center items-center p-12"><Loader className="w-8 h-8 animate-spin text-blue-600" /></div>
                  ...
                  {observations.map(obs => {
                    ...
                    return (
                        <div key={obs.id} className="bg-white p-4 rounded-lg border grid grid-cols-6 gap-4 items-center">
                          <div className="col-span-1 flex items-center justify-center"><div className="bg-gray-100 p-3 rounded-full">{isModel ? <Box className="w-5 h-5 text-gray-600"/> : isVideo ? <Video className="w-5 h-5 text-gray-600"/> : <Camera className="w-5 h-5 text-gray-600"/>}</div></div>
                          <div className="col-span-3"><p className="font-semibold text-gray-800">{description}</p><p className="text-sm text-gray-600">{obs.note?.[0]?.text || 'No additional notes.'}</p><p className="text-xs text-gray-400 mt-1">Date: {new Date(obs.effectiveDateTime).toLocaleDateString()}</p></div>
                          <div className="col-span-1 text-center"><span className={`text-xs font-medium px-2 py-1 rounded-full ${obs.status === 'final' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>{obs.status}</span></div>
                          <div className="col-span-1 text-right"><button onClick={() => handleViewModel(obs)} className="btn-secondary text-sm w-20 text-center" disabled={isGeneratingUrl === obs.id}>{isGeneratingUrl === obs.id ? <Loader className="w-4 h-4 animate-spin mx-auto"/> : 'View'}</button></div>
                        </div>
                    );
                  })}
                </div>
